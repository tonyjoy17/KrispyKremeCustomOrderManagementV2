      ...
      <div *ngIf="!loading" class="table-card">
        ...
        <div *ngIf="orders.length > 0">
          ...
          <div class="table-row" *ngFor="let order of orders">
            ...
            <span>
              <select class="status-select status-{{ order.status }}" [ngModel]="order.status" (ngModelChange)="updateStatus(order, $event)" (click)="$event.stopPropagation()">
...
export class FactoryOrdersComponent implements OnInit {
  ...
  setFilter(f: string) { this.activeFilter = f; this.currentPage = 1; this.loadOrders(); }
